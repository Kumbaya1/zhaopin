...
<div id="app">
    <div id="search">
        <input type="text" class="form-control" v-model='enName'  placeholder='输入企业名称'>
        <button class="btn btn-primary" @click='searchEn'>搜索企业</button>
    </div>
    <div id="infor-list">
        <table  class="table table-striped">
            <tr>
                <th class="title">企业名称</th>
                <th class="tel">联系电话</th>
                <th class="address">地址</th>
                <th class="date">所属行业</th>
                <th class="date">操作</th>
            </tr>
            <tr v-for='obj in showList'>
                <td class="title"><a href="javascript:;" v-text='obj.en_name' @click='showEn(obj)'></a></td>
                <td class="tel" v-text='obj.tel'></td>
                <td class="address">{{obj.prov}}省{{obj.city}}市</td>
                <td class="date" v-text='obj.industry'></td>
                <td class="operation"><a href="javascript:;" @click='delEn(obj.e_id)'>注销</a></td>
            </tr>
        </table>
    </div>
...
<script>
    var vm = new Vue({
        ...
        created:function(){
            var _this = this;
            $.get('Admin/get_en',function(data){
                _this.showList = data;
                _this.allList = data;
            },'json')
        },
        methods:{
            ...
            searchEn:function(){
                ...
                this.allList.forEach(function(obj,index){
                    if(obj.en_name.indexOf(_this.enName) >= 0){
